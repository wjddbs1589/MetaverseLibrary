...
namespace Suncheon
{
    public class StageScreen : MonoBehaviour
    {
        ...
        void ScreenLoad()
        {            
            StartCoroutine(UTILS.Requset_HttpGetData($"{GameManager.Instance.defaultData.serviceUrl}{GameManager.Instance.defaultData.mngrScreenLoad}", $"screen_location={EventLoacation.광장중앙무대}", (jsonData) =>
            {
        ...
        private void SetEventObject(Response_EventList result)
        {
            ...
            foreach (var item in result.response_EventListDatas)
            {
                ...
                if (startTime <= NowDateTime && NowDateTime >= endTime && SpawnerPos.오천그린광장.ToString() == item.eventLocation)
                {
